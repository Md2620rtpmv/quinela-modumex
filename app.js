...
function renderQuinielaView(pid) {
  ...
  Object.keys(GROUPS).forEach(g => {
    ...
    GROUPS[g].forEach((m, i) => {
      ...
      html += `<div style="display:grid;grid-template-columns:1fr auto auto auto 1fr;align-items:center;gap:6px;padding:8px 12px;border-bottom:1px solid rgba(255,255,255,.03);">
        <div class="team-home" style="font-size:12px;">${getFlag(m[0])} ${getShortName(m[0])}</div>
        <div style="text-align:center;font-family:'Bebas Neue';font-size:15px;color:var(--gold);min-width:42px;">${ph}:${pa}</div>
        <div style="text-align:center;font-size:10px;color:var(--muted);min-width:30px;">vs<br>${rh !== '' ? rh + ':' + ra : '—'}</div>
