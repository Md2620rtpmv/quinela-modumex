  ...
  matches.forEach((m, i) => {
    ...
    html += `<div class="match-row">
      ...
      <div class="score-inputs">
        <input class="score-box" type="number" min="0" max="20" value="${r.h !== null && r.h !== undefined ? r.h : ''}" placeholder="-" id="res_${key}_h" onchange="saveResult('${key}')">
        <span class="score-sep">:</span>
        <input class="score-box" type="number" min="0" max="20" value="${r.a !== null && r.a !== undefined ? r.a : ''}" placeholder="-" id="res_${key}_a" onchange="saveResult('${key}')">
      </div>
...
function renderQuinielaView(pid) {
  ...
  Object.keys(GROUPS).forEach(g => {
    ...
    GROUPS[g].forEach((m, i) => {
      ...
      const ph = pred && pred.h !== '' && pred.h !== null && pred.h !== undefined ? pred.h : '?';
      const pa = pred && pred.a !== '' && pred.a !== null && pred.a !== undefined ? pred.a : '?';
      const rh = real && real.h !== '' && real.h !== null && real.h !== undefined ? real.h : '';
      const ra = real && real.a !== '' && real.a !== null && real.a !== undefined ? real.a : '';
      html += `<div class="match-row" style="padding:8px 12px;">
        <div class="team-home" style="font-size:12px;">${getFlag(m[0])} ${getShortName(m[0])}</div>
        <div style="text-align:center;font-family:'Bebas Neue';font-size:15px;color:var(--gold);">${ph}:${pa}</div>
        <div style="text-align:center;font-size:10px;color:var(--muted);">vs<br>${rh !== '' ? rh + ':' + ra : '—'}</div>
        <span class="pts-chip${pts === 0 ? ' zero' : ''}">${pts}</span>
        <div class="team-away" style="font-size:12px;">${getShortName(m[1])} ${getFlag(m[1])}</div>
      </div>`;
    });
    html += `</div>`;
  });
  ...
  gkeys.forEach((g, i) => {
    ...
    GROUPS[g].forEach((m, idx) => {
      ...
      content += `<div class="match-predict">
        ...
        <input class="predict-input" type="number" min="0" max="20" value="${pred.h !== '' && pred.h !== null && pred.h !== undefined ? pred.h : ''}" placeholder="-" id="q_${key}_h">
        <span class="score-sep">:</span>
        <input class="predict-input" type="number" min="0" max="20" value="${pred.a !== '' && pred.a !== null && pred.a !== undefined ? pred.a : ''}" placeholder="-" id="q_${key}_a">
